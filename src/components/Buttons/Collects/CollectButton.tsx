  ...
  useEffect(() => {
    ...
    return (
      ...
      {alreadyCollected ? (
       ...
       <FolderPlusIcon className="w-4 md:h-4 text-[#96de26] font-bold"  />
       </div>
      ) : (
        <div 
        onClick={() => {setShowCollectModal(true)}} 
        className="flex items-center border md:border-none bg-[#96de26]
         ...
         shadow px-2 py-1 text-xs text-emerald-700 opacity-0 group-hover:opacity-100"> Collect </span>
         </div>
        )}
        <Modal
          title={
