...
const CollectButton: FC<Props> = ({publication}) => {
  ...
  useEffect(() => {
    ...
    return (
      ...
      {alreadyCollected ? (
       <div onClick={() =>{setShowCollectModal(true)}} className="flex items-center drop-shadow-lg  border border-[#96de26] md:border-none md:bg-emerald-700  rounded-full p-2 md:p-3">
       <FolderPlusIcon className="w-4 h-4 text-[#96de26] font-bold"  />
        </div>
      ) : (
        <div onClick={() => {setShowCollectModal(true)}} className="flex items-center drop-shadow-lg  border border-[#96de26] md:border-none md:bg-emerald-700 rounded-full p-2 md:p-3 md:hover:bg-[#96de26] group relative w-max">
            <FolderPlusIcon className='w-4 h-4 text-[#96de26] font-bold md:text-white'  />
