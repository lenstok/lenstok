...
const CollectButton: FC<Props> = ({publication}) => {
  ...
  useEffect(() => {
    ...
    return (
      ...
      {alreadyCollected ? (
       <div onClick={() =>{setShowCollectModal(true)}} className="flex items-center drop-shadow-lg  border border-[#96de26] md:border-none bg-black md:bg-emerald-700 rounded-full p-2 md:p-3">
       <FolderPlusIcon className="w-6 h-6 md:w-4 md:h-4 text-[#96de26] font-bold"  />
        </div>
      ) : (
        <div onClick={() => {setShowCollectModal(true)}} className="flex items-center drop-shadow-lg border border-[#96de26] md:border-none bg-black md:bg-emerald-700 rounded-full p-2 md:p-3 md:hover:bg-[#96de26] group relative w-max">
            <FolderPlusIcon className='w-6 h-6 md:w-4 md:h-4 text-[#96de26] font-bold md:text-white'  />
            <span className="hidden md:block pointer-events-none absolute -bottom-7 left-7 w-max shadow px-2 py-1 text-xs text-emerald-700 opacity-0 group-hover:opacity-100"> Collect </span>
          </div>
        )}
        <Modal
          title={
